...
&nbsp;&nbsp;(8) SPI Master 4个  
&nbsp;&nbsp;&nbsp;&nbsp;默认实例化其中 1 个，连接用户 SPI Flash（其中地址 0x0000_0000 ~ 0x0000_FFFF 用于存放用户程序，地址 0x0001_0000 存放目前在低 64KB 空间中存放的程序大小，其余空间可供用户使用），另外 3 个可供用户自定义使用  
&nbsp;&nbsp;(9) 支持中断控制器控制系统总中断  
  
  
### 本实验采用的 SoC 的地址分配  
  
**<center>![图 2.1.2 YADAN SoC 的地址分配](imgs/img_02_01_02.png)  
图 2.1.2 YADAN SoC 的地址分配</center>**
...
YADAN SoC 支持 9 个外部中断，3 个事件，在每个中断和事件发生时，内核会跳转到相应的程序地址里开始执行中断 / 事件处理程序。表 2.1.1 列出了各个中断 / 事件的入口地址，即中断向量表 (Interrupt Vector Table, IVT)。  
  
**<center>表 2.1.1 中断 / 事件 向量表**  
| 描述                         | 地址                      |
| ---------------------------- | ------------------------- |
...
### 外设相关的寄存器介绍  
从前文的图 2.1.2 中可以看到，在这个 SoC 中，外设通过 APB 总线与 CPU 核相连，外设的地址范围是 0x4A10_0000 ~ 0x4A11_1000，每个外设被分配了 4KB 的地址空间以供内核进行访问。在后文，本文将分别对每个外设进行介绍。  
...
#### 中断 / 事件控制模块  
YADAN SoC 最大支持 32 个矢量中断和 32 个事件，中断线和事件线分别被隔离和缓冲。本实验所用到的 SoC 默认定义了 9 个中断和 3 个事件。表 2.1.17 列出了各个中断 / 事件控制寄存器的功能。  
**<center>表 2.1.17 中断 / 事件控制寄存器总览**  
| Address     | R / W | Register Mnemonic | Function                |
| ----------- | ----- | ----------------- | ----------------------- |
...
##### EER (Event Enable)  
事件允许寄存器。每一位可被用于控制对应的事件的允许与禁用。  
  
##### EPR (Event Pending)  
事件挂起寄存器。每一位可被用于读 / 写对应的事件当前是否处于挂起状态。  
  
##### ESP (Event Set Pending)  
事件挂起设置寄存器。每一位可被用于触发对应的事件。  
  
##### ECP (Event Clear Pending)  
事件挂起清除寄存器。每一位可被用于清除对应的处于挂起状态的事件。  
  
##### SCR (Sleep Control)  
休眠控制寄存器。将最低位设置为 1 可以让内核进入休眠。当中断或事件出现时，内核将再次被唤醒。  
<center>
...
**<center>![图 2.2.3 本实验采用的 SoC 的地址分配](imgs/img_02_02_03.png)  
图 2.2.3 本实验采用的 SoC 的地址分配</center>**
...
Zero-riscy 支持 9 个外部中断，3 个事件，在每个中断和事件发生时，内核会跳转到相应的程序地址里开始执行中断 / 事件处理程序。表 2.2.1 列出了各个中断 / 事件的入口地址，即中断向量表 (Interrupt Vector Table, IVT)。  
  
**<center>表 2.2.1 中断 / 事件 向量表**  
| 描述                         | 地址                      |
| ---------------------------- | ------------------------- |
| 保留-未使用                  | 0x0000_0000 ~ 0x0000_0058 |
| 中断 23 ：I²C IRQ            | 0x0000_005c               |
...
#### 中断 / 事件控制模块  
PULPino 最大支持 32 个矢量中断和 32 个事件，中断线和事件线分别被隔离和缓冲。本实验所用到的 SoC 默认定义了 9 个中断和 3 个事件。表 2.2.17 列出了各个中断 / 事件控制寄存器的功能。  
**<center>表 2.2.17 中断 / 事件控制寄存器总览**  
| Address     | R / W | Register Mnemonic | Function                |
| ----------- | ----- | ----------------- | ----------------------- |
